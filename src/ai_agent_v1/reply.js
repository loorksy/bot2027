...
function getTemplateReply(type, context) {
    ...
    switch (type) {
        ...
        // === NEW: Salary Delay Response ===
        case 'SALARY_DELAY':
            return `بفهم قلقك ${d.dear}${e('heart')}\n\nحالياً نحن نعمل على تسليم الرواتب لجميع العملاء. يمكنك متابعة حالة راتبك من صفحتك الشخصية:\n\n${context.portalUrl || '(اكتبي "رابط" للحصول على رابطك)'}\n\n${e('check')} سترين صورة الوصل هناك فور إتمام التحويل\n${e('bell')} وستصلك رسالة تأكيد عند وصول الوصل\n\nالله يسهل عليكي${e('star')}`;

        // === NEW: Salary Amount Query ===
        case 'SALARY_AMOUNT_QUERY':
            return `${d.dear}، مبلغ الراتب يتم حسابه بناءً على أيام العمل والخصومات (إن وجدت)${e('think')}\n\nيمكنك مراجعة تفاصيل راتبك كاملة من صفحتك الشخصية:\n${context.portalUrl || '(اكتبي "رابط" للحصول على رابطك)'}\n\nإذا كان هناك خطأ فعلاً، اكتبي "طلب مراجعة الراتب" وسنتحقق من الأمر ${e('check')}`;

        // === NEW: Receipt Status ===
        case 'RECEIPT_STATUS':
            return `صور الوصولات تُرفع مباشرة على صفحتك الشخصية ${d.dear}${e('happy')}\n\n${context.portalUrl || '(اكتبي "رابط" للحصول على رابطك)'}\n\nمن هناك بتقدري:\n• تشوفي جميع الوصولات\n• تحمليها على جهازك\n\n${e('bell')} عند رفع وصل جديد ستصلك رسالة تنبيه`;

        // === NEW: Support Ticket Created ===
        case 'TICKET_CREATED':
            return `تم إنشاء طلب جديد برقم: ${context.ticketNumber}${e('check')}\n\n${d.dear}، سيتواصل معك أحد المسؤولين قريباً إن شاء الله.\n\nشكراً لصبرك${e('heart')}`;

        // === NEW: Data Update (Direct) ===
        case 'DATA_UPDATE_SUCCESS':
            return `${d.good}!${e('check')} تم تحديث ${context.fieldName} بنجاح!\n\nالقيمة الجديدة: ${context.newValue}\n\nفي شي تاني ${d.dear}؟`;

        // === NEW: Data Update (As Ticket) ===
        case 'DATA_UPDATE_TICKET':
            return `تم إرسال طلب تعديل البيانات للإدارة ${d.dear}${e('check')}\n\nرقم الطلب: ${context.ticketNumber}\n\nسيتم مراجعة طلبك والتواصل معك قريباً.\nشكراً لصبرك${e('heart')}`;

        // === NEW: Knowledge Base Response ===
        case 'KNOWLEDGE_RESPONSE':
            let kbResponse = context.answer;
            if (context.sendPortalLink && context.portalUrl) {
                kbResponse += `\n\n🔗 صفحتك الشخصية:\n${context.portalUrl}`;
            }
            return kbResponse;
